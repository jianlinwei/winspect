...
//const char DRIVER_FLAT_BUFFER_TAG[] = "bfeS";

const bool configInspectionOutbound = TRUE; // Направление просмотра TCP стрима 1 - Inbound, 0 - Outbound.
const USHORT configInspectionPort = 0; // 5001 Порт, который будет инспектироваться.
...
void basic_wfpUnload(IN PDRIVER_OBJECT DriverObject)
{
	...
	if(gCalloutIdV4 != NULL && gCalloutIdV6 != NULL) UnregisterCallout();
	// Здесь нужно вызвать код освобождения ресурсов WFP.
   ...
   OUT UINT32* calloutId)
{
   ...
   filterConditions[0].fieldKey = (configInspectionOutbound ? FWPM_CONDITION_IP_REMOTE_PORT : // В каком направлении смотреть?
                                                        FWPM_CONDITION_IP_LOCAL_PORT);
   filterConditions[0].matchType = FWP_MATCH_GREATER_OR_EQUAL; 
   filterConditions[0].conditionValue.type = FWP_UINT16; 
   filterConditions[0].conditionValue.uint16 = configInspectionPort; // Порт, на котором будем смотреть.
...
NTSTATUS RegisterCallout(
   IN const PVOID gUnref,               // Моя модификация, т.к. параметр не используется.
   IN void* deviceObject)
{
   NTSTATUS status = STATUS_SUCCESS;
...
{
	if(gDBG_Mode) DbgPrint("New packet on port\n");
	...
	*/
	...
	// Здесь нужно разобрать NET_BUFFER_LIST, чтобы получить данные.

	/* FWPS_STREAM_DATA* packetData = ioPacket->streamData;
	size_t dataLength = packetData->dataLength;
	size_t dataOffset = packetData->dataOffset;
	NET_BUFFER_LIST* nblList = packetData->netBufferListChain;
    */ 
	//
		...
		(streamData->flags & FWPS_STREAM_FLAG_RECEIVE_EXPEDITED))
	{
	 ...
	 // Здесь нужно извлечь информацию из пакета и показать.
